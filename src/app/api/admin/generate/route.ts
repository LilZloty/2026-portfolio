...
- MAXIMUM 1200 characters (about 200 words)
- LinkedIn cuts off at 3 lines - hook must be in first 2 lines
- Short paragraphs only
- If you go over 1200 characters, START OVER and write shorter
...
2. Story-based - "Last week..." (keep it brief)
3. Quick tip - 3-4 lines max
4. Data drop - Lead with surprising stat
5. The breakdown - 3-4 bullet points max

## FORMAT:
- Line 1-2: Hook (gets cut off after this)
- Line 3-6: Main point with ONE example
- Line 7-8: CTA or question

## ABSOLUTE RULES:
- ZERO emojis - absolutely critical
- Use dashes (-) not bullets
- NO long paragraphs
- NO comprehensive guides - just ONE insight
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Rate limiting - expensive operations (more strict for content generation)
    const clientId = getClientIdentifier(request);
    const rateLimit = checkRateLimit(clientId, 'admin-generate', { maxRequests: 5, windowMs: 60 * 1000 });
    ...
    const { topic, context, messages, outputs, drafts } = body;
    ...
    const paths: { blog?: string; linkedin?: string; twitter?: string } = {};
    ...
    if (outputs.blog) {
      let blogContent = '';
      if (drafts?.blog) {
        blogContent = ensureMdxFormat(drafts.blog, topic, date);
      } else {
        const blogPrompt = BLOG_PROMPT.replace('{DATE}', date);
        blogContent = await generateWithGrok(blogPrompt, fullContext);
      }
      ...
      paths.blog = `content/blog/${slug}.mdx`;
    }

    // Generate LinkedIn if requested
    if (outputs.linkedin) {
      const linkedinContent = drafts?.linkedin || await generateWithGrok(LINKEDIN_PROMPT, fullContext);
      ...
      paths.linkedin = `content/social/linkedin-${slug}.txt`;
    }

    // Generate Twitter if requested
    if (outputs.twitter) {
      const twitterContent = drafts?.twitter || await generateWithGrok(TWITTER_PROMPT, fullContext);
      ...
      paths.twitter = `content/social/twitter-${slug}.txt`;
    }
